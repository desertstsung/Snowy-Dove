;+
; object for individual gf6 wfv sensor
; date 2020/2/18
;-
...
pro oWFV6::_setLastFile, fn
  ...
  self.files[(WHERE(self.files ne ''))[-1]] = fn
end

pro oWFV6::appendFile
  compile_opt idl2, hidden
...
function oWFV6::getLastFile
  compile_opt idl2, hidden

  RETURN, self.files[(WHERE(self.files ne ''))[-1]]
end

pro oWFV6::setProperty, tgz_fn = tgz_fn, $
...
pro oWFV6::getProperty, tgz_fn = tgz_fn, $
  wrkDir = wrkDir, imgDir = imgDir, $
  calGain = calGain, calOffs = calOffs, $
  wvl = wvl, flag = flag, files = files
  compile_opt idl2, hidden
...
function oWFV6::init, tgz_fn, info, flag, o_fn
  compile_opt idl2, hidden
...
pro oWFV6__define
  ...
  structure = { oWFV6, inherits IDL_Object, $
