 ...
 *    --File format convert model for linux-based platform
 *  Corresponding to ../binary/sd_nv2tiff
...
void main(int argc, char *argv[]){
    ...
    if (filesizeingb < 4) {
        ...
        entrytmp.count          = nb;
        entrytmp.data_or_offset = (_long) pos.__pos;    //windows uses pos directly, linux uses pos.__pos instead
        ...
        if (*inter == "bsq") {
            ...
            entrytmp.dataType       = 4;    //long
            entrytmp.count          = nb;
            entrytmp.data_or_offset = (_long) pos.__pos;
            ifd->stripOffsets       = entrytmp;
            fwrite(arraySO, LONG_LEN * nb, 1, pTFf);
            fgetpos(pTFf, &pos1);
            fsetpos(pTFf, &pos);
            for (_short i = 0; i < nb; i++) {
                arraySO[i] = (_long) pos1.__pos + (_long) i * ns * nl * (dt == 4 ? FLOAT_LEN : SHORT_LEN);
            }
            fwrite(arraySO, LONG_LEN * nb, 1, pTFf);
            free(arraySO);
            void *data;
        ...
        ifh->offsetOfIFD = (_long) pos.__pos;
        fwrite(ifh, IFH_LEN, 1, pTFf);
        ...
        if (nb > 4) {    //entry stores data's offset
            ...
            fwrite(arrayBPS, SHORT_LEN * nb, 1, pTFf);
            free(arrayBPS);
            entrytmp.data_or_offset = (_long8) pos.__pos;    //windows uses pos directly, linux uses pos.__pos instead
        ...
        if (*inter == "bsq") {
            ...
            entrytmp.count          = nb;
            entrytmp.data_or_offset = (_long8) pos.__pos;
            ifd->stripOffsets       = entrytmp;
            fwrite(arraySO, LONG8_LEN * nb, 1, pTFf);
            fgetpos(pTFf, &pos1);
            fsetpos(pTFf, &pos);
            for (_short i = 0; i < nb; i++) {
                arraySO[i] = (_long8) pos1.__pos + (_long8) i * ns * nl * (dt == 4 ? FLOAT_LEN : SHORT_LEN);
            }
            fwrite(arraySO, LONG8_LEN * nb, 1, pTFf);
            free(arraySO);
            void *data;
        ...
        fseek(pTFf, 0, SEEK_SET);
        ifh->offsetOfIFD = (_long8) pos.__pos;
        fwrite(ifh, BIG_IFH_LEN, 1, pTFf);
