 ...
 *    --NDVI calculating model for windows platform
 *  Corresponding to ../binary/sd_ndvi.exe
    ...
    if (*inter == "bsq") {
        ...
        if (dt == 2) {
            ...
            for(_long i = 0; i < nl; i++) {
                pos.__value = (_long8) ns * (i + 2 * nl) * SHORT_LEN;
                fsetpos(pnvf, &pos);
                fread(red, (_long8) SHORT_LEN * ns, 1, pnvf);
                pos.__value = (_long8) ns * (i + 3 * nl) * SHORT_LEN;
                fsetpos(pnvf, &pos);
            ...
            float  *nd; nd  = (float *) malloc((_long8) FLOAT_LEN * ns);
            for(_long i = 0; i < nl; i++) {
                pos.__value = (_long8) ns * (i + 2 * nl) * FLOAT_LEN;
                fsetpos(pnvf, &pos);
                fread(red, (_long8) FLOAT_LEN * ns, 1, pnvf);
                pos.__value = (_long8) ns * (i + 3 * nl) * FLOAT_LEN;
                fsetpos(pnvf, &pos);
